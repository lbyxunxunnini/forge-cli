...
tmp_session_project="$(mktemp -d -t forge-cli-session.XXXXXX)"
tmp_session_name="$(basename "$tmp_session_project")"
mkdir -p "$tmp_session_project/.claude/.forge-cli/projects"
printf 'project_guardrails:\n  project:\n    name: "%s"\n    root_type: "existing"\n' "$tmp_session_name" > "$tmp_session_project/.claude/.forge-cli/projects/${tmp_session_name}.project_guardrails.yaml"
session_init_output="$(scripts/ff_session.sh --project-root "$tmp_session_project" init --track execution --phase S2 --mode 页面开发)"
printf '%s\n' "$session_init_output" | grep -q "$tmp_session_project/.claude/.forge-cli/session.md" || fail "ff_session did not use project_guardrails host session path"
...
grep -q '^- 等待状态：artifact' "$tmp_session_project/.claude/.forge-cli/session.md" || fail "ff_session did not persist waiting_state"
grep -q '^- 等待输入类型：screenshot' "$tmp_session_project/.claude/.forge-cli/session.md" || fail "ff_session did not persist expected_input"
rm -rf "$tmp_session_project"
info "session path and waiting-state validation passed"

tmp_resume_project="$(mktemp -d -t forge-cli-resume.XXXXXX)"
tmp_resume_name="$(basename "$tmp_resume_project")"
mkdir -p "$tmp_resume_project/.claude/.forge-cli/projects"
printf 'project_guardrails:\n  project:\n    name: "%s"\n    root_type: "existing"\n' "$tmp_resume_name" > "$tmp_resume_project/.claude/.forge-cli/projects/${tmp_resume_name}.project_guardrails.yaml"
...
grep -q '^- 等待状态：none' "$tmp_resume_project/.claude/.forge-cli/session.md" || fail "ff_session did not clear waiting_state after consume-resume"
grep -q '\[f-forge\] 等待：等待用户确认改动契约' "$tmp_resume_project/.forge-cli/runtime/session_events.log" || fail "ff_session did not append waiting event"
grep -q '\[f-forge\] 恢复等待：已消费用户补充输入，继续当前阶段。' "$tmp_resume_project/.forge-cli/runtime/session_events.log" || fail "ff_session did not append resume-consumed event"
