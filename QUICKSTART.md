...
> **触发方式**：输入 `/forge-cli`（推荐）或 `ff-`（快捷）开头的消息。
...
适合新页面、新模块、路由接入、状态接入。Forge CLI 会先收口结构，再实现。

## 3. 先扫描项目生成 project_guardrails

```text
ff- 这是一个迭代中的 项目。先扫描项目结构，输出 project_guardrails，不要先写代码。
```
...
```text
ff- 我想做一个新的 项目，先帮我收口需求、风格和首批页面结构。
```
...
```bash
scripts/doctor.sh
scripts/validate_project.sh /path/to/project/app
python3 scripts/project_snapshot.py /path/to/project/app
python3 scripts/init_project_guardrails.py /path/to/project/app --profile auto --interactive
```
