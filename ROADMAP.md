...
> 通用 AI 协作 CLI Agent
> 目标：达到 Claude Code 70%+ 核心体验，支持多语言、多模型、多工具、可挂载工作流
> 更新时间：2026-06-15
>
...
| Phase 6: 工作流系统 | ✅ 完成 | 100% | 工作流进入/退出、/workflows 交互选择、/exit-wf、语义退出、状态栏 |
| Phase 7: flutter-forge 接入 | ✅ 完成 | 100% | 原生 TS 移植、15 模块、5 角色子 agent、并行派发、11 gate、3 种模式 |
| Phase 8: h5-forge 接入 | ✅ 完成 | 100% | 原生 TS 移植、15 模块、5 角色子 agent、并行派发、11 gate、3 种模式、H5/Web 技术栈检测 |
...
| 工作流进入/退出 | ✅ | activeWorkflow 状态、/workflows 列表选择、/exit-wf、语义退出检测 |
| flutter-forge 工作流 | ✅ | 原生 TS 移植、5 角色子 agent 隔离、并行派发、11 gate、3 种模式 |
| h5-forge 工作流 | ✅ | 原生 TS 移植、5 角色子 agent 隔离、并行派发、11 gate、3 种模式、H5/Web 技术栈检测 |
...
| flutter-forge 接入 | P2 | 2 周 | ✅ | 原生 TS 移植为标准工作流实例，15 模块，完整编排能力 |
| h5-forge 接入 | P2 | 1 天 | ✅ | 原生 TS 移植为标准工作流实例，15 模块，H5/Web 技术栈检测 |
...
**下一步重点**：中期计划全部完成！进入长期计划：插件市场、IDE 集成、更多工作流。
...
| 2026-06-15 | **工作流系统增强**：新增 activeWorkflow 持久状态、/workflows 交互式列表选择、/exit-wf 命令、语义退出检测、状态栏显示当前工作流 |
| 2026-06-15 | **flutter-forge 原生移植**：完整移植为标准工作流实例（v0.4.0），15 个 TypeScript 模块，5 角色子 agent 隔离，结构化 JSON 传递，架构师并行派发，11 gate 校验，3 种模式（standard/fast/autonomous），方案 B 记忆体系 |
| 2026-06-15 | **h5-forge 原生移植**：完整移植为标准工作流实例（v0.4.0），15 个 TypeScript 模块，5 角色子 agent 隔离，H5/Web 技术栈检测（React/Vue/Next/Nuxt/Angular/Svelte），5 个技术栈 profiles，11 gate 校验，3 种模式 |
...
### 核心模块
- `src/agents/orchestrator.ts` - 主控编排器（含 activeWorkflow 工作流状态管理）
- `src/agents/pool.ts` - Agent 并行执行池
- `src/tools/` - 工具系统（22 个工具）
- `src/memory/manager.ts` - 记忆系统（含子目录支持）
- `src/workflows/` - 外部工作流引擎（manifest.yaml 驱动）
- `src/workflows/executor.ts` - 工作流执行器（含 flutter-forge、h5-forge 原生集成）
- `src/workflows/registry.ts` - 工作流注册表
- `src/hooks/` - Hooks 系统
- `src/plugins/` - 插件系统
- `src/mcp/` - MCP 支持
- `src/security/` - 安全检查
...
### flutter-forge 工作流（`~/.forge-cli/workflows/flutter-forge/`）
- `manifest.yaml` - 工作流声明
- `scripts/controller.ts` - 总控（路由→guardrails→phase→role→gate→并行）
- `scripts/types.ts` - 核心类型定义
- `scripts/gate-checker.ts` - 11 个 gate 校验
- `scripts/phase-engine.ts` - S0-S6/C0-C3 状态机
- `scripts/role-dispatcher.ts` - 子 agent 隔离执行
- `scripts/parallel-executor.ts` - 按模块并行派发
- `references/` - 36 个参考文档
- `references/roles/` - 5 个角色 prompt
- `profiles/` - 5 个技术栈模板

### h5-forge 工作流（`~/.forge-cli/workflows/h5-forge/`）
- `manifest.yaml` - 工作流声明
- `scripts/controller.ts` - H5ForgeController 总控
- `scripts/types.ts` - H5ForgeSession、TechStackConfig（H5/Web 技术栈）
- `scripts/project-scanner.ts` - 检测 package.json、React/Vue/Next/Nuxt 等
- `scripts/task-classifier.ts` - h5f-/h5f-fast/h5f-a 触发词
- `scripts/gate-checker.ts` - 11 个 gate 校验
- `scripts/phase-engine.ts` - S0-S6/C0-C3 状态机
- `scripts/role-dispatcher.ts` - 子 agent 隔离执行
- `scripts/parallel-executor.ts` - 按模块并行派发
- `references/` - 16 个参考文档
- `references/roles/` - 5 个角色 prompt
- `profiles/` - 5 个技术栈模板（react-zustand、vue-pinia、nextjs-app-router、react-redux-toolkit、nuxt3）
...
Phase 5: 功能完善         ███████████████████░  90%
Phase 6: 工作流系统       ████████████████████ 100%
Phase 7: flutter-forge    ████████████████████ 100%
Phase 8: h5-forge         ████████████████████ 100%
中期计划                   ████████████████████ 100%
```
...
| 代码操作 | 10 项 | ✅ 全部完成 |
| CLI 命令 | 27 个 | ✅ 全部完成 |
| 工作流 | 2 个（flutter-forge、h5-forge） | ✅ 完整移植 |
| 测试用例 | 79 个 | ✅ 全部通过 |
| 示例文件 | 5 个 | ✅ 全部完成 |
| 文档 | 11 份 | ✅ 大部分完成 |
...
3. **长期计划**（1-2 月）
   ...
   - IDE 集成
   - 更多工作流
   - flutter-forge / h5-forge 实际使用验证

---

*最后更新：2026-06-15*

*项目已从 Forge CLI 重构为通用 Forge CLI Agent，支持挂载外部工作流*
